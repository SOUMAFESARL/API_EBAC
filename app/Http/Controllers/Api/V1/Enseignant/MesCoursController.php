...
class MesCoursController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        if (! $annee) {
            return response()->json(['message' => 'Aucune année académique active ou en cours.', 'matieres' => [], 'meta' => $this->meta(0, 1, 15)]);
        }

        $query = $this->affectations($request, $annee->id)
    ...
    public function show(Request $request, int $matiere): JsonResponse
    {
        ...
        abort_unless($annee, 404, 'Aucune année académique active ou en cours.');
        $affectations = $this->affectations($request, $annee->id)->where('id_matiere', $matiere)->get();
        abort_if($affectations->isEmpty(), 404, 'Cette matière ne vous est pas affectée pour cette année académique.');

        return response()->json(['matiere' => $this->matiere($affectations, $annee, true)]);
    }

    private function affectations(Request $request, int $anneeId)
    {
        return AffectationEnseignant::query()->where('enseignant_id', $request->user()->id)->where('id_annee_academique', $anneeId)
