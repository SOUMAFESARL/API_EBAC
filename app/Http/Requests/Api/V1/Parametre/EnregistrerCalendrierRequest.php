...
class EnregistrerCalendrierRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'jours_feries.*' => ['array:libelle,date'],
            'jours_feries.*.libelle' => ['string', 'max:180'],
            'jours_feries.*.date' => ['date_format:Y-m-d', 'distinct'],
            'conges' => ['present', 'array', 'list', 'max:100'],
            'conges.*' => ['array:libelle,date_debut,date_fin'],
            'conges.*.libelle' => ['string', 'max:180'],
